...
function minMovesToStables(reindeer, stables) {
    //Primero ordeno los dos arrays para quitarme de problemas con tener que recorrer todo el array para ver cual es el mas cercano
    reindeer.sort((a, b) => a - b);
    stables.sort((a, b) => a - b);
    console.log("Renos en posiciones: "+reindeer);
    console.log("Establos en posiciones: "+stables);
    let movimientos = 0;
    //Voy sumando los movimientos por cada paso del array
    for (let i = 0; i < reindeer.length; i++) {
        console.log(i+1+"er reno: se mueve de la posicion"+reindeer[i]+" al establo en la posicion "+stables[i]);
